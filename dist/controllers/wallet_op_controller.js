...
const annotation_controller_1 = __importDefault(require("./annotation_controller"));
var OperationType;
...
const remove = async (user_id, operation_id) => {
    try {
        ...
        await op_delete_map[data[0].operation_type](user_id, data[0].value);
        if (error)
            throw error;
        if (data[0].annotation_id) {
            await updateAnnByRemoveOp(user_id, data[0].annotation_id);
        }
        return { data: data[0], status: 200, error, message: 'sucessfully deleted wallet operation' };
    }
    catch (error) {
...
const updateAnnByRemoveOp = async (user_id, annotation_id) => {
    try {
        const { data, error } = await annotation_controller_1.default.get(user_id, annotation_id);
        if (error)
            throw error;
        if (!data)
            throw (0, error_system_1.getNewResponseError)('annotation not Found', 404);
        await annotation_controller_1.default.uncheckStatus(user_id, annotation_id);
...
const removeByBulkAnnotation = async (user_id, reduceResult) => {
    try {
        await wallet_controller_1.default.add(user_id, reduceResult.value);
        const { data, error } = await supabase_1.default.from('wallet_operations')
